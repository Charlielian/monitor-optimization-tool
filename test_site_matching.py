...
mock_hsr_info = [
    {
        'id': 1,
        'line_name': '京沪高铁',
        'Transmitting_Point_Name': '北京南站',
        'area': '北京',
        'site_type': '高铁站点',
        'bbu_name': 'BBU_BJSZ_01',
        'celname': 'BJSZ_4G_01',
    ...
    },
    {
        'id': 3,
        'line_name': '京沪高铁',
        'Transmitting_Point_Name': '天津西站',
        'area': '天津',
        'site_type': '高铁站点',
        'bbu_name': 'BBU_TJZX_01',
        'celname': 'TJZX_4G_01',
...
mock_alarms = [
    ...
    {
        'alarm_code_name': '超级小区CP退服',
        'alarm_title': '超级小区控制面故障',
        'alarm_level': '紧急',
        'occur_time': datetime.now(),
        'alarm_reason': '超级小区CP退服，CPID: 4，逻辑小区ID: 460-00-12672955-1，错误码: 1-200。',
        'ne_id': '12672955',
        'alarm_object_name': '北京南站',
        'additional_info': 'CPID: 4，逻辑小区ID: 460-00-12672955-1，NR gNBId:12672955。'
    }
]
...
# 告警匹配函数
def match_alarm_to_site(alarm, hsr_info):
    """匹配告警到发射点"""
    # 转换hsr_info数据格式
    cells = []
    for cell in hsr_info:
        cells.append({
            'id': cell['id'],
...
# 测试告警匹配到发射点
def test_alarm_to_site_matching():
    """测试告警匹配到发射点"""
    print("\n" + "="*100)
    print("开始测试高铁告警匹配到发射点")
    print("="*100)
    
    print("\n加载hsr_info表数据:")
    print("-"*100)
    for cell in mock_hsr_info:
        print(f"发射点: {cell['Transmitting_Point_Name']} | 小区: {cell['celname']} | CGI: {cell['CGI']} | CPID: {cell['cpId']}")
    
    total_alarms = len(mock_alarms)
    matched_alarms = 0
    ...
    for i, alarm in enumerate(mock_alarms):
        ...
        # 匹配告警到发射点
        matched_sites = match_alarm_to_site(alarm, mock_hsr_info)
