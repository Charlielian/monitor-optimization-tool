...
def create_app() -> Flask:
    ...
    # 如果启用了SSL，自动将SESSION_COOKIE_SECURE设置为True
    session_cookie_secure = cfg.security_config.get("session_cookie_secure", False)
    if cfg.ssl_config.get("enabled", False):
...
if __name__ == "__main__":
    ...
    ssl_context = None
    if cfg and cfg.ssl_config.get('enabled', False):
        cert_file = cfg.ssl_config.get('cert_file')
        key_file = cfg.ssl_config.get('key_file')
        if cert_file and key_file:
            ssl_context = (cert_file, key_file)
            print(f"🔒 已启用HTTPS，使用证书: {cert_file}")
        else:
            print("⚠️ SSL已启用但证书或密钥文件未配置")
