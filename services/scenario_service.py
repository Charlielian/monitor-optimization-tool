                        ...
                        if use_time_range:
                            ...
                            sql = f"""
                                SELECT
                                    ...
                                    -- 4G切换成功率和掉线率（使用原始公式）
                                    SUM(COALESCE("HO_SuccOutInterFreq",0) + COALESCE("HO_SuccOutIntraFreq",0)) * 100.0 / NULLIF(SUM(COALESCE("HO_AttOutInterFreq",0) + COALESCE("HO_AttOutIntraFreq",0)), 0) AS avg_ho_success_rate,
                                    SUM(COALESCE("Wireless_Drop_Cnt",0)) * 100.0 / NULLIF(SUM(COALESCE("RRC_SuccConnEstab",0)), 0) AS avg_drop_rate,
                    ...
                    results.append(
                        {
                            ...
                            "切换成功率(%)": round(row.get("avg_ho_success_rate") or 0, 2),
                            "无线掉线率(%)": round(row.get("avg_drop_rate") or 0, 2),
                        ...
                        if use_time_range:
                            ...
                            sql = f"""
                                SELECT
                                    ...
                                    -- 5G切换成功率和掉线率
                                    SUM(COALESCE("HO_SuccOutInterCuNG",0) + COALESCE("HO_SuccOutInterCuXn",0) + COALESCE("HO_SuccOutIntraCUInterDU",0) + COALESCE("HO_SuccOutIntraDU",0)) * 100.0 / NULLIF(SUM(COALESCE("HO_AttOutInterCuNG",0) + COALESCE("HO_AttOutInterCuXn",0) + COALESCE("HO_AttOutIntraCUInterDU",0) + COALESCE("HO_AttOutIntraDU",0)), 0) AS avg_ho_success_rate,
                                    0 AS avg_drop_rate,
                                    SUM("RRC_ConnMax") AS total_rrc_users,
            ...
            if network == "4G":
                ...
                sql = f"""
                    SELECT 
                        ...
                        -- 4G切换成功率和掉线率（使用原始公式）
                        (COALESCE("HO_SuccOutInterFreq",0) + COALESCE("HO_SuccOutIntraFreq",0)) * 100.0 / NULLIF(COALESCE("HO_AttOutInterFreq",0) + COALESCE("HO_AttOutIntraFreq",0), 0) AS ho_success_rate,
                        COALESCE("Wireless_Drop_Cnt",0) * 100.0 / NULLIF(COALESCE("RRC_SuccConnEstab",0), 0) AS drop_rate,
                ...
                sql = f"""
                    SELECT 
                        ...
                        -- 5G切换成功率和掉线率
                        (COALESCE("HO_SuccOutInterCuNG",0) + COALESCE("HO_SuccOutInterCuXn",0) + COALESCE("HO_SuccOutIntraCUInterDU",0) + COALESCE("HO_SuccOutIntraDU",0)) * 100.0 / NULLIF(COALESCE("HO_AttOutInterCuNG",0) + COALESCE("HO_AttOutInterCuXn",0) + COALESCE("HO_AttOutIntraCUInterDU",0) + COALESCE("HO_AttOutIntraDU",0), 0) AS ho_success_rate,
                        0 AS drop_rate,
                        "RRC_ConnMax" as rrc_users,
                        interference,
                        GREATEST(
            ...
            for cgi, cell_info in cell_map.items():
                if cgi in metrics_map:
                    ...
                    all_cells.append({
                        ...
                        "ho_success_rate": metrics.get("ho_success_rate") or 0,
                        "drop_rate": metrics.get("drop_rate") or 0,
