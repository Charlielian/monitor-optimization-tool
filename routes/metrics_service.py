...
class MetricsService:
    ...
    def get_table_name(self, network_type: str, granularity: str = "15m", query_type: str = "hour") -> str:
        """根据网络类型、粒度和查询类型返回表名
        
        Args:
            network_type: "4G" 或 "5G"
            granularity: "15m" (15分钟) 或 "1h" (1小时)
            query_type: "hour" (小时指标) 或 "day" (日指标)
        ...
        if query_type == "day":
            # 日指标表
            if network_type == "4G":
                return "cell_4g_metrics_day"
            else:
                return "cell_5g_metrics_day"
        else:
            # 小时指标表（原有逻辑）
            return self._table_map.get(
    ...
    def cell_timeseries_bulk(
        ...
        Args:
            ...
            granularity: 粒度 (15m/1h)
            query_type: 查询类型 (hour/day)
        """
        if not cell_ids:
            return []
        ...
        if network_type == "4G":
            table_4g = self.get_table_name("4G", granularity, query_type)
        ...
        table_5g = self.get_table_name("5G", granularity, query_type)
        placeholders = ",".join(["%s"] * len(ids))
        sql = f"""
            SELECT
                start_time,
    ...
    def cell_timeseries_mixed(
        ...
        if cgi_4g:
            data_4g = self.cell_timeseries_bulk(cgi_4g, "4G", start, end, granularity, query_type)
            for row in data_4g:
                row['network_type'] = '4G'
        ...
        if cgi_5g:
            data_5g = self.cell_timeseries_bulk(cgi_5g, "5G", start, end, granularity, query_type)
            for row in data_5g:
                row['network_type'] = '5G'
                if 'cgi' not in row or not row.get('cgi'):
                    row['cgi'] = row.get('cell_id')
        ...
        return all_data, auto_detected
                ...
                if network_type == "4G":
                    return "cell_4g_metrics"
                else:
                    return "cell_5g_metrics"
    ...
    def daily_traffic_and_voice(self, target_date: datetime | None = None) -> Dict[str, Any]:
        ...
        result = {
            ...
            "4G": {"traffic_gb": 0, "traffic_tb": 0, "traffic_unit": "GB", "voice_erl": 0, "voice_unit": "Erl"},
            "5G": {"traffic_gb": 0, "traffic_tb": 0, "traffic_unit": "GB", "voice_erl": 0, "voice_unit": "Erl"},
        }
    ...
    def daily_traffic_and_voice_by_region(self, target_date: datetime | None = None) -> Dict[str, Any]:
        ...
        result = {
            "date": target_date.strftime("%Y-%m-%d"),
            "4G": {region: {"traffic_gb": 0, "traffic_tb": 0, "traffic_unit": "GB", "voice_erl": 0, "voice_unit": "Erl"} for region in regions},
            "5G": {region: {"traffic_gb": 0, "traffic_tb": 0, "traffic_unit": "GB", "voice_erl": 0, "voice_unit": "Erl"} for region in regions},
        }
        
        # 查询4G数据（话务量不除以10000，保持Erl单位，同时获取CGI）
        sql_4g = """
            SELECT 
                cellname,
                cgi,
        ...
        for row_4g in rows_4g:
            ...
            traffic_gb = float(row_4g.get("上下行总业务量GB", 0) or 0)
            voice_erl = float(row_4g.get("VoLTE话务量Erl", 0) or 0)
            
            result["4G"][region]["traffic_gb"] += traffic_gb
            result["4G"][region]["voice_erl"] += voice_erl
        ...
        # 查询5G数据（话务量不除以10000，保持Erl单位，同时获取CGI）
        sql_5g = """
            SELECT 
                userlabel AS cellname,
                "Ncgi" AS cgi,
        ...
        for row_5g in rows_5g:
            ...
            traffic_gb = float(row_5g.get("RLC层总流量GB", 0) or 0)
            voice_erl = float(row_5g.get("Vonr话务量Erl", 0) or 0)
            
            result["5G"][region]["traffic_gb"] += traffic_gb
            result["5G"][region]["voice_erl"] += voice_erl
