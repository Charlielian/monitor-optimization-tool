...
def grid_detail(grid_id):
    ...
    try:
        # 获取网格小区信息
        grid_cells = grid_service.get_grid_cells(grid_id)
        cell_4g_count = len(grid_cells.get('4g', []))
        cell_5g_count = len(grid_cells.get('5g', []))
        ...
        # 获取小区统计信息
        cell_stats = grid_service.get_grid_cell_stats(grid_id)
        
        # 时间范围和粒度显示
        time_range_display = "当天"
        granularity_display = "小时级"
        
        # 网格信息（从小区数据中提取）
        grid_info = None
        grid_name = None
        if grid_cells.get('4g'):
            grid_name = grid_cells['4g'][0].get('grid_name')
        elif grid_cells.get('5g'):
            grid_name = grid_cells['5g'][0].get('grid_name')
        ...
        return render_template(
            ...
            cell_4g_count=cell_4g_count,
            cell_5g_count=cell_5g_count,
            daily_traffic=daily_traffic,
            latest_hour_metrics=latest_hour_metrics,
        ...
        logging.error(f"网格详情页面渲染失败: {e}", exc_info=True)
        flash(f"获取网格详情失败: {str(e)}", "danger")
        return redirect(url_for("grid.grid"))
